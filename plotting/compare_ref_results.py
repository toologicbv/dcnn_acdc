...
def compare_referral_results_v2(ref_result_objects, width=16, height=10, plot_base=False, x_range=None, y_range=None,
    ...
    num_of_res_objects = len(ref_result_objects)
    ...
    for phase in np.arange(2):
        ...
        # fig.suptitle(str_phase, **config.title_font_large)
        # confusing but true: ax2 is the first plot in a row
        ...
        ax3 = plt.subplot2grid((rows, columns), (0, 4), rowspan=2, colspan=2, sharey=ax2)
        ax1.set_ylim([min_dice, max_dice])
        ax2.set_ylim([min_dice, max_dice])
        ax3.set_ylim([min_dice, max_dice])
        ...
        if plot_type == "perc":
            ...
            ax3.plot(x_t, np.ones(x_t.shape[0]), linestyle=":", c="gray", linewidth=6)
            ...
            ax3.set_xticks(np.array([0.001, 0.1, 1, 10, 100.]))
            ...
            ax3.set_xlabel("Mean % of voxels", **config.axis_font28)
        else:
        ...
        for res_idx, ref_result_obj in enumerate(ref_result_objects):
            ...
            mdl_marker = model_markers[layout_idx]
            mdl_linestyle = model_linestyles[layout_idx]
            if loss_function == "brier":
                f_axis = ax2
            elif loss_function == "soft-dice":
                f_axis = ax1
            elif loss_function == "cross-entropy":
                f_axis = ax3
            f_axis.set_title("{} ({})".format(loss_function.title(), str_phase), **config.title_font_large)
            ...
            for cls in np.arange(1, 4):
                ...
                if plot_type == "perc":
                    ...
                    max_cls_dice = np.max(X[:, class_idx])
                l_label = "{}-{}".format(type_of_map, class_lbls[cls])
                ...
                if plot_type == "perc":
                    ...
                    xs = 100 * (all_per_values[:, phase] + eps)  # ES % values per referrence threshold
                    # xs = np.concatenate((xs, [0]))
                    # y_values = np.concatenate((X[:, cls], [dice_wo_ref[0, cls]]))
                    y_values = X[:, class_idx]
                    if np.max(xs) < 100.:
                ...
                wo_ref.fill(dice_wo_ref[phase, cls])
                # plot class-dice for all thresholds
                f_axis.plot(xs, y_values, label=l_label, c=color_code[cls],
                ...
                if max_cls_dice is not None:
                    f_axis.plot(xs_max_line, y_max_line, c=color_code[cls], linestyle=mdl_linestyle,
                             alpha=0.15, linewidth=6)
                # plot our baseline performance
                if plot_base:
                    f_axis.plot(x_axis, wo_ref, c=color_code[cls], linestyle=model_linestyle_base, alpha=0.05, linewidth=6)

                f_axis.legend(loc=4, fontsize=24)
                f_axis.grid("on")
        ...
        if do_save:
            ...
            filename = fig_name + "_" + str_phase + "_" + plot_type
            # fig_name_pdf = os.path.join(fig_path, filename + ".pdf")
            fig_name_jpeg = os.path.join(fig_path, filename + ".jpeg")

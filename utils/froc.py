...
def compute_slice_detection_performance(patient_pred_probs_dict, patient_gt_labels_dict, nbr_of_thresholds=40,
                                         range_threshold=[0, 1.], base_apex_labels=None, loc=0):

    """

    :param patient_pred_probs_dict:
    :param patient_gt_labels_dict:
    :param nbr_of_thresholds:
    :param range_threshold:
    :param base_apex_labels:
    :param loc:
    :return:
    """
    ...
    for i, threshold in enumerate(threshold_list):
        ...
        for patient_id, pred_probs_slices in patient_pred_probs_dict.iteritems():
            gt_labels_slices = patient_gt_labels_dict[patient_id]
            # store all labels for this patient
            pat_pred_labels = []
            pat_gt_labels = []
            pat_reg_pred_labels = []
            pat_reg_gt_labels = []
            for slice_id, slice_pred_probs in pred_probs_slices.iteritems():
            ...
            # print(type(pat_reg_pred_labels), pat_reg_pred_labels.shape, type(pat_reg_gt_labels), pat_reg_gt_labels.shape)
    ...
    return mean_sensitivity[::-1], mean_precision[::-1], mean_fp_rate[::-1], mean_fp[::-1], mean_reg_sensitivity[::-1], \
           mean_reg_precision[::-1], mean_reg_fp_rate[::-1], mean_reg_fp[::-1]
